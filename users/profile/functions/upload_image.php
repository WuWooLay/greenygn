<?php
        ...
        if($check = @getimagesize($file)) {
            ...
            if( in_array($ext, $allowed) ) {
                ...
                if( count($req['errors']) == 0 ) {
                    ...
                    // Declare Patch
                    $path = __DIR__ . "/../../../assets/images/users/";
                    // New Name
                    ...
                    if( floor($h/$w) != 1 ) {
                        ...
                        switch ($ext) {
                            ...
                            if (
                                ...
                                if (save_image($newName)) { $_SESSION['user']['image'] = "/assets/images/users/" . $newName;$req['success'][] = "Successfully Uploaded";}
                        ...
                        if(move_uploaded_file($file, $path . $newName)) {
                            $sql = "UPDATE `users` SET `image` = '".  "/assets/images/users/" . $newName ;
                            $sql .= "' WHERE `users`.`id` = " . $_POST['id'];
                            // die($sql);
                            if(mysqli_query($conn, $sql))  {
                                $_SESSION['user']['image'] = "/assets/images/users/" . $newName;
    ...
    function save_image($image_url) {
        global $conn;
        $sql = "UPDATE `users` SET `image` = '/assets/images/users/".  $image_url ;
        $sql .= "' WHERE `users`.`id` = " . $_POST['id'];
